    ...
    <div className="p-6 max-w-7xl mx-auto space-y-8" data-testid="dashboard-page">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Welcome back{user?.name ? `, ${user.name}` : ""}</p>
        </div>
        <Link href="/borrowers">
          <button className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-lg text-sm font-medium hover:bg-slate-800 transition-colors" data-testid="button-add-borrower">
            Add Borrower <ArrowUpRight className="h-4 w-4" />
          </button>
        </Link>
      </div>
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <StatCard title="Total Principal" value={formatCurrency(stats?.totalPrincipal ?? 0)} sub="Deployed capital" icon={IndianRupee} />
            <StatCard title="Total Interest Earned" value={formatCurrency(stats?.totalInterestEarned ?? 0)} sub="All time" icon={TrendingUp} accent />
            <StatCard title="Total Commission" value={formatCurrency(stats?.totalCommissionEarned ?? 0)} sub="Above 10% rate" icon={Percent} accent />
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard title="This Month - Interest" value={formatCurrency(stats?.currentMonthInterest ?? 0)} sub="Base interest earned" icon={CalendarCheck} />
            <StatCard title="This Month - Commission" value={formatCurrency(stats?.currentMonthCommission ?? 0)} sub="Commission earned" icon={Percent} accent />
            <StatCard title="Pending Payments" value={String(stats?.pendingPaymentsCount ?? 0)} sub="Unpaid this cycle" icon={AlertCircle} />
            <StatCard title="Closed / Defaulted" value={`${stats?.closedBorrowers ?? 0} / ${stats?.defaultedBorrowers ?? 0}`} sub="Closed and defaulted" icon={Users} />
          </div>

          {/* Interest vs Commission breakdown */}
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="text-center p-4 bg-slate-50 rounded-xl border border-slate-100">
                  ...
                  <p className="text-2xl font-bold text-slate-900">{formatCurrency(stats?.currentMonthInterest ?? 0)}</p>
                </div>
                <div className="text-center p-4 bg-slate-50 rounded-xl border border-slate-100">
                  <p className="text-xs font-medium text-slate-500 uppercase tracking-wide mb-1">Base Interest (10%)</p>
                  <p className="text-2xl font-bold text-slate-700">{formatCurrency(stats?.currentMonthBaseInterest ?? 0)}</p>
                </div>
                <div className="text-center p-4 bg-emerald-50 rounded-xl border border-emerald-100">
                  <p className="text-xs font-medium text-emerald-600 uppercase tracking-wide mb-1">Commission (above 10%)</p>
                  <p className="text-2xl font-bold text-emerald-700">{formatCurrency(stats?.currentMonthCommission ?? 0)}</p>
                </div>
              </div>
            </CardContent>
          </Card>

          {/* Recent active borrowers */}
          <Card className="border-slate-200">
                ...
                <div className="divide-y divide-slate-100">
                  {(borrowers ?? []).slice(0, 5).map(b => (
                    <Link key={b.id} href={`/borrowers/${b.id}`}>
                      <div className="flex items-center justify-between px-6 py-4 hover:bg-slate-50 transition-colors cursor-pointer" data-testid={`row-borrower-${b.id}`}>
                        <div>
